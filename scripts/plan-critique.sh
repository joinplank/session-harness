...
TEMPLATE="$REPO_DIR/scripts/plan-reviewer.md"
...
case "$CRITIC" in
  codex)  codex exec -m "${CODEX_MODEL:-gpt-5.6-sol}" -c model_reasoning_effort="$EFFORT" --cd "$REPO_DIR" -s read-only -o "$RAW" "$PROMPT" </dev/null >/dev/null 2>&1 \
            || { echo "codex critic failed to run — escalate, do not guess a verdict" >&2; exit 1; } ;;
  claude) claude -p --dangerously-skip-permissions "$PROMPT" </dev/null >"$RAW" 2>/dev/null \
