...
```bash
SLUG="<slug>"
DIR="$(scripts/session.sh new --slug "$SLUG" --launch --message "$ARGUMENTS")"   # prints the clone path
```
...
that happens in the cmux-rooted Claude, which has its own full context. Run `scripts/session.sh
list` to see open sessions; `scripts/session.sh release <slug>` resets a finished one to a warm
