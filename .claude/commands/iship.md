...
```
/goal Ship issue #N via .claude/commands/iship.md. Done when `scripts/session.sh status` reports READY (a non-draft PR closes #N) or ESCALATED (interactive:escalated). Run it to prove which. Or stop after 20 turns.
```

**The completion bar, stated once (this is what the terse goal keys on):** the run is done at one
of two terminal states, each surfaced by `scripts/session.sh status` as a single transcript token
...
what the transcript shows, so steps 3 and 4 **run `scripts/session.sh status`** at their terminal
...
.claude/skills/adversarial-review/run-review.sh --reviewer codex --round "$ROUND" --out "$W/review.md"
```

This writes **two** files: `$W/review.md` — COMPACT (the classifier's blocking findings + the
`VERDICT:` line) — and `$W/review.full.md` — the reviewer's final review message (the raw session
transcript is never published). **Read only `$W/review.md`** to assess and fix; it's all you need
under the severity rule. **Never `Read` `review.full.md` into your context** — it exists solely to
`cat` into the PR `<details>` below for provenance. That split is what keeps this session's
context clean across review rounds.
...
**Assessing a round.** Read `$W/review.md` (compact — its `VERDICT:` line + blocking findings),
...
```bash
ROUND=<n>; VERDICT="$(grep -E '^VERDICT:' "$W/review.md" | tail -1)"
{
  ...
  cat "$W/review.full.md"   # final Codex review straight to the PR — bash pipe, never into your context
  printf '\n</details>\n'
} > "$W/round-comment.md"
...
delta with a fresh-context checker (prompt: `scripts/concept-reviewer.md`) — it judges concepts
...
```bash
scripts/concept-check.sh --out "$W/concepts.md"    # ends VERDICT: MINIMAL | SIMPLIFY
```
...
```bash
scripts/session.sh status    # prints "ESCALATED — issue #N labeled interactive:escalated …"
```
...
scripts/session.sh status    # prints "READY — non-draft PR #… closes #N …" into the transcript
```

Leave the PR for the **human to merge** — never merge it yourself. After they merge (closes #N),
clean up the clone with **`scripts/session.sh reap`** (or `/reap`): it confirms the PR merged +
...
To free the slot yourself without waiting for reap, `scripts/session.sh release <slug> --force`
